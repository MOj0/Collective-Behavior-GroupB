...
For example Heppner and Grenader \cite{Heppner_nonlinear_model} were modeling birds behavior with stohastic nonlinear differential equations.
...
Our research is based on a paper by Papadopoulou and others \cite{papadopoulou2022emergence}, which we will extend with the results our predator and prey simulation.
Although we are not using fuzzy logic to set the direction and speed of our boids, which makes the movement less natural, we have taken taken some elements for our model from
\cite{JDemsar_predator_attacks}. Specifically, we've set the field of vision to our boids to 300° and implemented occulsion for the predator.
