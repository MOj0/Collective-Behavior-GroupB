...
\begin{enumerate}
	\item position - a vector in $ R^2 $, denoted by $position(B)$
	\item velocity - a vector in $ R^2 $, denoted by $velocity(B)$
	\item acceleration - a vector in $ R^2 $, is not used by the behaviour logic, but only for (correct) internal computation of the boid's velocity
\end{enumerate}

With regards to behaviour logic, each boid has:
\begin{enumerate}
